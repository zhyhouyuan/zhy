...
public:
    ...
    void run()
    {
        ...
        double l_n2n=0,r_n2n=0,n2n_num=0;
                ...
                double lp_hit1=0,lp_hit1_filter = 0;
                double rp_hit1=0,rp_hit1_filter = 0;
                //double mid_p_n=0,mid_p_n_filter = 0;
                int hit_n = 10;
	...
	//	fclose(f_e1_e2);
                ...
                cout<<lp_n_filter<<endl;
                cout<<"left:"<<endl;
                cout<<" MR:      "<<lsum/fb_l.size()<<'\t'<<"   hit@10:    "<<lp_n/fb_l.size()<<'\t'<<"  hit@1:    "<<lp_hit1/fb_l.size()<<endl;
                cout<<"MR_filter:"<<lsum_filter/fb_l.size()<<'\t'<<"hit@10_filter:"<<lp_n_filter/fb_l.size()<<'\t'<<"hit@1_filter:"<<lp_hit1_filter/fb_l.size()<<endl;

                cout<<"right:"<<endl;
                cout<<" MR:      "<<rsum/fb_r.size()<<'\t'<<"   hit@10:    "<<rp_n/fb_r.size()<<'\t'<<"  hit@1:    "<<rp_hit1/fb_r.size()<<endl;
                cout<<"MR_filter:"<<rsum_filter/fb_r.size()<<'\t'<<"hit@10_filter:"<<rp_n_filter/fb_r.size()<<'\t'<<"hit@1_filter:"<<rp_hit1_filter/fb_r.size()<<endl;


              //  cout<<"right:"<<rsum/fb_r.size()<<'\t'<<rp_n/fb_r.size()<<'\t'<<rp_hit1/fb_r.size()<<'\t'<<rsum_filter/fb_r.size()<<'\t'<<rp_n_filter/fb_r.size()<<'\t'<<rp_hit1_filter/fb_r.size()<<endl;
...
void prepare()
{
	...
	while (fscanf(f_kb2,"%s",buf)==1)
    {
        ...
        test.add(entity2id[s1],entity2id[s2],relation2id[s3],0);
    }
    fclose(f_kb2);

    FILE* f_kb3 = fopen("../data/test.txt","r");
